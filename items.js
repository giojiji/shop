...
let findgenre = myshop.map(filtereditems => {
  if (filtereditems.title.toUpperCase().includes(searchbar.value.toUpperCase()) || 
  filtereditems.category.toUpperCase().includes(searchbar.value.toUpperCase())
  ) {
...
if(document.getElementsByClassName('productAPIbox').length < 19 && searchbar.value.length != 0) {
...
let findgenre = myshop.map(movie => {
  if (movie.category == checkboxarr[0] || 
...
function changingprice() {
  ...
  document.getElementById("pricefrom").innerText = "price from: " + rangevalue + " $"
  let findgenre = myshop.map(movie => {
